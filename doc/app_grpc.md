说明
----
此应用模式目前在试验阶段，只支持基本的[grpc协议](https://grpc.io/)，另外需要配合使用nginx的ngx_http_v2_module来支持http2（虽然在php层没强制要求http2，但是要使用grpc官方提供的各语言client的话则是需要支持http2的）
...
service_schemes配置定义grpc应用所需要服务描述文件的加载方式。
