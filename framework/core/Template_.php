...
class Template
{
    ...
    {
        return self::readText(self::readTag(self::readInclude($str), $end));
    }
    ...
    protected static function readInner($str, &$ck)
    {
        return preg_replace_callback(self::tagRegex('inner'), function ($matches) use (&$tpls) {
            $attrs = self::parseTagAttrs($matches, ['name']);
            $ck[]  = $attrs['name'];
    ...
    protected static function readInclude($str)
    {
        return preg_replace_callback(self::tagRegex('include'), function ($matches) {
            ...
            $attrs = self::parseTagAttrs($matches, ['name', $vname]);
            if (isset($attrs['name'])) {
                $arg = '"'.$attrs['name'].'"';
            } else {
                $val = self::readValue($attrs[$name]);
                $arg = self::readExp($val['code'], $val['vars']);
            }
    ...
    protected static function readExtends($str, &$ck)
    {
        ...
        $name = self::parseTagAttrs($matches[0][0][0], ['name'])['name'];
        $ck[] = $name;
        // 读取子模版block
        ...
        if (preg_match_all($regex, $str, $matches, PREG_OFFSET_CAPTURE)) {
            ...
            foreach ($matches[1] as $i => $match) {
                ...
                if ($matches[1][$i][0] == self::$config['assign_attr_prefix']) {
                    ...
                    $code[] = self::wrapCode("$$name = ".self::readExp(substr($val, 1, -1)).';');
                } else {
                    $count++;
                    ...
                    $code[] = self::wrapCode(self::readStruct($attr, substr($val, 1, -1)));
                }
     ...
     * 解析标签属性
     */  
    protected static function parseTagAttrs($str, $attrs = null, $self_end = true)
    {
        if ($self_end !== null) {
            if ($self_end) {
                if (substr($str, -2) != '/>') {
                ...
                if (substr($str, -2) == '/>') {
                    throw new TemplateException("parseTagAttrs error: 不允许自闭合标签 $str");
                }
            }
        }
        $prefix = preg_quote(self::$config['arg_attr_prefix'].self::$config['assign_attr_prefix']
